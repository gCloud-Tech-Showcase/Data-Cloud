...
> **Note:** The `.env` file (generated by `terraform apply`) includes `GOOGLE_GENAI_USE_VERTEXAI=TRUE` and `GOOGLE_CLOUD_LOCATION=us-central1` for the AI agent. Ensure you have Application Default Credentials set up: `gcloud auth application-default login`
